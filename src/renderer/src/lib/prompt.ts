...
- Merging is allowed and often better than picking a survivor: if two bullets under the same role each bury one JD-relevant, metric-bearing element in otherwise-weak padding, pull the two strong elements together into one new bullet rather than keeping one whole bullet and discarding the other's good part. A merge counts as exactly one bullet against the cap. The one hard condition: only merge elements that are genuinely true of the same underlying project or effort. Never combine fragments from two different, unrelated pieces of work into one bullet — that implies they happened together, which is a fabrication even though every individual fact in it is real.
...
> BEFORE: <my current text — if this is a merge, list both original bullets>
> AFTER: <your proposed text>
> WHY: <which JD skill or phrase this now hits, and one line confirming it stays true to my resume — if this is a merge, also confirm the merged elements are genuinely from the same underlying project or effort, not two different ones stitched together>
